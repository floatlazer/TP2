// Produit matrice-vecteur parallèle par ligne
# include <cassert>
# include <vector>
# include <iostream>
# include <fstream>
...
public:
    ...
    Matrix( int nrows, int ncols, int debrows );
...
// ---------------------------------------------------------------------
Matrix::Matrix( int nrows, int ncols, int debrows ) : m_nrows(nrows), m_ncols(ncols),
                                         m_arr_coefs(nrows*ncols)
{
    int dim = (nrows > ncols ? nrows : ncols );
    for ( int i = 0; i < nrows; ++ i ) {
        for ( int j = 0; j < ncols; ++j ) {
            (*this)(i,j) = (i+j+debrows)%dim;
        }
    }    
}
// =====================================================================
int main( int nargs, char* argv[] )
{
    ...
    const int debRow = Nloc*rank;
    Matrix A(Nloc, N, debRow);
    out  << "A : " << A << std::endl;
    std::vector<double> u( N );
    for ( int i = 0; i < N; ++i ) u[i] = i+1;
    out << " u : " << u << std::endl;
    std::vector<double> vLoc = A*u;
    std::vector<double> v(N);
    MPI_Allgather(vLoc.data(), Nloc, MPI_DOUBLE, v.data(), Nloc, MPI_DOUBLE, dup );
    out << "A.u = " << v << std::endl;
    out.close();
    MPI_Finalize();
    return EXIT_SUCCESS;
